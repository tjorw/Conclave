...
public class UpdateProgramTagDefinitionHandlerTests
{
    private readonly IEditionRepository _editionRepo = Substitute.For<IEditionRepository>();
    private readonly IConventionRepository _conventionRepo = Substitute.For<IConventionRepository>();
    private readonly ICurrentUser _currentUser = Substitute.For<ICurrentUser>();
    private readonly UpdateProgramTagDefinitionHandler _handler;

    public UpdateProgramTagDefinitionHandlerTests()
    {
        _handler = new UpdateProgramTagDefinitionHandler(_editionRepo, _conventionRepo, _currentUser);
    }
    ...
    [Fact]
    public async Task Handle_ValidCommand_UpdatesTagDefinition()
    {
        var (_, admin, edition) = Setup();
        edition.AddProgramTagDefinition("Barn");
        _currentUser.PersonId.Returns(admin.Id);

        await _handler.Handle(new UpdateProgramTagDefinitionCommand(edition.Id.Value, "Barn", "Barnvänligt"), default);
    ...
    public async Task Handle_PerformerNotAdministrator_Throws()
    {
        ...
        await Assert.ThrowsAsync<ForbiddenException>(
            () => _handler.Handle(new UpdateProgramTagDefinitionCommand(edition.Id.Value, "Barn", "Tagg"), default));
    }
}
