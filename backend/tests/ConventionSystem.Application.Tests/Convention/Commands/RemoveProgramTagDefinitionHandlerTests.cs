...
public class RemoveProgramTagDefinitionHandlerTests
{
    private readonly IEditionRepository _editionRepo = Substitute.For<IEditionRepository>();
    private readonly IConventionRepository _conventionRepo = Substitute.For<IConventionRepository>();
    private readonly ICurrentUser _currentUser = Substitute.For<ICurrentUser>();
    private readonly RemoveProgramTagDefinitionHandler _handler;

    public RemoveProgramTagDefinitionHandlerTests()
    {
        _handler = new RemoveProgramTagDefinitionHandler(_editionRepo, _conventionRepo, _currentUser);
    }
    ...
    [Fact]
    public async Task Handle_ValidCommand_RemovesTagDefinition()
    {
        var (_, admin, edition) = Setup();
        edition.AddProgramTagDefinition("Barnvänligt");
        _currentUser.PersonId.Returns(admin.Id);

        await _handler.Handle(new RemoveProgramTagDefinitionCommand(edition.Id.Value, "Barnvänligt"), default);
    ...
    public async Task Handle_PerformerNotAdministrator_Throws()
    {
        ...
        await Assert.ThrowsAsync<ForbiddenException>(
            () => _handler.Handle(new RemoveProgramTagDefinitionCommand(edition.Id.Value, "Barnvänligt"), default));
    }
}
