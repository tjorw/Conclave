...
public class DeactivatePersonHandlerTests
{
    private readonly IConventionRepository _conventionRepo = Substitute.For<IConventionRepository>();
    private readonly IPersonRepository _personRepo = Substitute.For<IPersonRepository>();
    private readonly DeactivatePersonHandler _handler;

    public DeactivatePersonHandlerTests()
    {
        _handler = new DeactivatePersonHandler(_conventionRepo, _personRepo);
    }
    ...
    [Fact]
    public async Task Handle_ActivePerson_DeactivatesPerson()
    {
        var (_, person) = Setup();

        await _handler.Handle(new DeactivatePersonCommand(person.Id.Value), default);
    ...
    public async Task Handle_PersonNotFound_Throws()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => _handler.Handle(new DeactivatePersonCommand(Guid.NewGuid()), default));
    }

    [Fact]
    public async Task Handle_AlreadyInactivePerson_Throws()
    {
