...
public sealed class EditionRepository(ConventionDbContext db) : IEditionRepository
{
    ...
    public Task<Edition?> GetByIdWithContentAsync(EditionId id, CancellationToken ct = default)
        => db.Editions
            .Include(e => e.Content)
            .FirstOrDefaultAsync(e => e.Id == id, ct);

    public Task<Edition?> GetByIdWithLocalesAsync(EditionId id, CancellationToken ct = default)
        => db.Editions
            .Include(e => e.Locales)
            .FirstOrDefaultAsync(e => e.Id == id, ct);
