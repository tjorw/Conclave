...
public class ConfirmAssignmentHandlerTests
{
    private readonly IShiftRepository _shiftRepo = Substitute.For<IShiftRepository>();
    private readonly IEditionRepository _editionRepo = Substitute.For<IEditionRepository>();
    private readonly IConventionRepository _conventionRepo = Substitute.For<IConventionRepository>();
    private readonly ConfirmAssignmentHandler _handler;

    public ConfirmAssignmentHandlerTests()
    {
        _handler = new ConfirmAssignmentHandler(_shiftRepo, _editionRepo, _conventionRepo);
    }
    ...
    [Fact]
    public async Task Handle_ValidCommand_ConfirmsAssignment()
    {
        var (_, admin, _, shift) = Setup();
        var assignment = shift.AssignPerson(PersonId.New(), admin.Id);

        await _handler.Handle(new ConfirmAssignmentCommand(shift.Id.Value, assignment.Id.Value, admin.Id.Value), default);
    ...
    public async Task Handle_ShiftNotFound_Throws()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => _handler.Handle(new ConfirmAssignmentCommand(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid()), default));
    }
    ...
    public async Task Handle_PerformerNotAuthorized_Throws()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => _handler.Handle(new ConfirmAssignmentCommand(shift.Id.Value, assignment.Id.Value, nonAdmin.Id.Value), default));
    }
    ...
    public async Task Handle_StaffCoordinatorCanConfirm()
    {
        ...
        await _handler.Handle(new ConfirmAssignmentCommand(shift.Id.Value, assignment.Id.Value, staffCoordId), default);
    ...
    public async Task Handle_StaffAreaResponsibleCanConfirm()
    {
        ...
        await _handler.Handle(new ConfirmAssignmentCommand(shift.Id.Value, assignment.Id.Value, areaResponsibleId), default);
